...
Le cours SDD IV, comme son nom l'indique, est essentiellement axé vers le développement de pratiques qui rendront vos analyses reproductibles. Cela signifie que quelqu'un d'autre pourra reprendre votre code et générer exactement les mêmes résultats, tableaux et graphiques.
...
Avant de vous lancer dans ce cours, vous devez installer et vous familiariser avec la SciViews Box `r learnitdown$svbox`. Si ce n'est pas encore fait, retournez à la [page d'accueil pour bien débuter](%60r%20learnitdown$baseurl%60).

RStudio et (R) Markdown ne doivent plus avoir de secret pour vous.
...
L'ensemble de ces outils a été abordé dans les cours précédents [SDD I](%60r%20paste(learnitdown$baseurl,%20%22sdd-umons%22,%20sep%20=%20%22/%22)%60), [SDD II](%60r%20paste(learnitdown$baseurl,%20%22sdd-umons2%22,%20sep%20=%20%22/%22)%60) et SDD III (les six premiers modules du présent cours) que vous devez avoir réalisé avant d'entamer la matière du cours SDD IV : pratique.
...
```{r}
fmutate(df, date = make_date(annee, mois, jour))
```
...
```{r assign_D07Ia_21M_data, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("D07Ia_21M_data", part = "I",
    url = "https://github.com/BioDataScience-Course/D07Ia_21M_data",
    course.ids = c(
      'S-BIOG-043'         = !"D07Ia_{YY}M_data"),
    course.urls = c(
      'S-BIOG-043'         = "https://classroom.github.com/a/mmcdbtb6"),
    course.starts = c(
...
La problématique est de convertir une chaîne de caractères en bits qui sera interprétée par l'ordinateur. Il faut donc attribuer à chaque caractère un code unique. Cette série de codes va permettre la traduction des caractères. Les fameux termes cités précédemment sont les encodages qui permettent cette traduction.

Historiquement, le code américain normalisé pour l'échange d'informations (*ASCII*, "American Standard Code for Information Interchange") a été l'un des premiers systèmes d'encodage proposés. Cet encodage se fait sur 8 bits par caractère, permettant ainsi 256 caractères maximum. Cette proposition était simple et adaptée uniquement à l'anglais. Il ne prenait par exemple pas en compte les accents de la langue française. Il s'en suit du développement de plusieurs systèmes d'encodages avec des adaptations spécifiques aux très nombreuses langues mondiales comme **ISO 8859** pour les langues latines (*ISO 8859-1*, *latin1*) ou encore ISO-2020 pour les langues asiatiques.
...
Cet exemple montre une fois de plus l'importance de lire l'aide d'une fonction et de tenter de faire des tests très simples pour s'assurer de bien comprendre la fonction et ses arguments.
...
-   qui se répète plusieurs fois : + **?** de 0 à 1 fois + **+** 1 ou plusieurs + \_\_\*\_\_ 0 ou plusieurs + **{2,6}** répétitions comprises entre 2 et 6 fois
-   **.**, n'importe quels caractères
-   \_\_.\*\_\_ n'importe quels caractères après de longueur quelconque
...
```{r}
swallows <- fmutate(swallows, heading = circular(heading,
  units = "degrees", template = "geographics"))
swallows
```
...
```{r assign_D07Ia_21M_data_II, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("D07Ia_21M_data", part = "II",
    url = "https://github.com/BioDataScience-Course/D07Ia_21M_data",
    course.ids = c(
      'S-BIOG-043'         = !"D07Ia_{YY}M_data"),
    course.urls = c(
