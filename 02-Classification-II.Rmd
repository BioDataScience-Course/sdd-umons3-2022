...
La matrice de confusion et les métriques qui en sont dérivées ne doivent plus avoir de secret pour vous pour évaluer les performances d'un classifieur dans la phase de test.
...
```{r assign_C02Ia_ml2_I, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("C02Ia_ml2", part = "I",
    url = "https://github.com/BioDataScience-Course/C02Ia_ml2",
    course.ids = c(
      'S-BIOG-025'         = !"C02Ia_{YY}M_ml2"),
    course.urls = c(
      'S-BIOG-025'         = "https://classroom.github.com/a/..."),
    course.starts = c(
      'S-BIOG-025'         = !"{W[6]+5} 08:00:00"),
    course.ends = c(
      'S-BIOG-025'         = !"{W[15]+5} 23:59:59"),
    term = "Q1", level = 3,
    toc = "Validation croisée en pratique")
```
...
Un seul paramètre doit donc être définit : *k*, représentant le nombre d'individus proches considérés. Un vote à la majorité permet de déterminer à quel groupe appartient l'objet testé. En cas d'ex aequo, la classe de l'individu du set d'apprentissage le plus proche est utilisée. Pour minimiser le risque d'ex aequo, *k* est généralement choisi impair. Dans notre schéma, nous utilisons *k* = 3, valeur qui s'est avérée optimale dans beaucoup de situations. La recherche de la valeur optimale de *k* dans le cadre de l'application finale sera évidemment possible ultérieurement.
...
Nous avons choisi arbitrairement de prendre un dictionnaire de taille 30 et 3 plus proches voisins. Avec ces valeurs, nous avons 24% d'erreur, ce qui nous situe entre k-NN et ADL en termes de performances. *Essayez par vous-même d'autres valeurs de `size =` et `k.nn =` pour optimiser encore ce résultat.*
...
```{r assign_C02Ia_ml2_II, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("C02Ia_ml2", part = "II",
    url = "https://github.com/BioDataScience-Course/C02Ia_ml2",
    course.ids = c(
      'S-BIOG-025'         = !"C02Ia_{YY}M_ml2"),
    course.urls = c(
...
Vous pouvez aussi visionner une [magnifique présentation animée de ce qu'est un arbre de partitionnement ou décisionnel](http://www.r2d3.us/lapprentissage-automatique-en-images-chapitre-1/) (pensez bien à sélectionner le français en tout début de document et faites défiler jusqu'à la fin de l'histoire). Dans cette présentation, la notion de **suréchantillonnage** (*oversampling* en anglais) est abordée. C'est un problème qui se rencontre souvent en classification supervisée, tout comme en régression. Dans le cas de l'arbre de partitionnement, la solution consiste à élaguer les branches de moindre importance de l'arbre. [Cette page explique de manière très claire de quoi il s'agit](http://www.r2d3.us/visual-intro-to-machine-learning-part-2/).
...
```{r assign_C02Ia_ml2_III, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("C02Ia_ml2", part = "III",
    url = "https://github.com/BioDataScience-Course/C02Ia_ml2",
    course.ids = c(
      'S-BIOG-025'         = !"C02Ia_{YY}M_ml2"),
    course.urls = c(
    ...
    course.ends = c(
      'S-BIOG-025'         = !"{W[15]+5} 23:59:59"),
    term = "Q1", level = 3,
    toc = "Classification par forêt aléatoire")
```
...
Vous allez maintenant discuter par équipe ces différents aspects. **Pour chaque équipe, un porte-parole sera désigné et acceptera le projet GitHub Classroom suivant en se plaçant dans le groupe unique proposé.** Il y entrera le rapport de synthèse de son équipe. Les autres étudiants peuvent également accepter le projet GoitHub Classroom et rentrer dans le même groupe afin de pouvoir visualiser son contenu, mais n'y effectueront aucune modification.

```{r assign_C02Ga_optimisation, echo=FALSE, results='asis'}
if (exists("assignment2"))
  assignment2("C02Ga_optimisation", part = NULL,
