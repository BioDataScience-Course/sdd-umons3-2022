...
Avant de vous lancer tête baissée dans de la matière nouvelle, assurez-vous d'avoir installé la SciViews Box `r learnitdown$svbox`. Si ce n'est pas encore fait, retournez à la [page d'accueil pour bien débuter](%60r%20learnitdown$baseurl%60).
...
L'ensemble de ces outils a été abordé dans les cours précédents [SDD I](%60r%20paste(learnitdown$baseurl,%20%22sdd-umons%22,%20sep%20=%20%22/%22)%60),

Outre les outils, ce cours s'appuie sur des notions que nous avons déjà abordé dans le cours de [science des données biologiques partie 2](%60r%20paste(learnitdown$baseurl,%20%22sdd-umons2%22,%20sep%20=%20%22/%22)%60) la notion de **classification non supervisée**. De même, la modélisation (via les **régressions**) a aussi fait l'objet d'une attention particulière dans le cours précédent. Toutes ces connaissances vont servir de pilier de base pour ce que nous étudierons ensemble dans ce module, ainsi que dans les deux suivants concernant la classification supervisée.
...
<!-- TODO: update this into the 2021-2022 branch, or place it elsewhere? (with the cheatsheets?) -->
...
Ce projet est un projet par groupe. **Les groupes doivent correspondre au rangée de la salle de cours.**

```{r assign_C01Ga_21_metrique, echo=FALSE, results='asis'}
if (exists("assignment2"))
  assignment2("C01Ga_21_metrique", part = NULL,
    url = "https://github.com/BioDataScience-Course/C01Ga_21_metrique",
    course.ids = c(
      'S-BIOG-025'         = !"C01Ga_{YY}M_metrique"),
    course.urls = c(
      'S-BIOG-025'         = "https://classroom.github.com/a/cJQ-Ve_U"),
    course.starts = c(
      'S-BIOG-025'         = !"{W[4]+3} 13:00:00"),
    course.ends = c(
      'S-BIOG-025'         = !"{W[4]+5} 23:59:59"),
    term = "Q1", level = 4, n = 4,
...
```{r}
penguins %<-% rename(penguins, # %<-% pour collecter" le resultat d'une fonctions tidy!
...
penguins %>.%
  ...
  drop_na(.) %->% # %->ù pour collecter les résultats
  penguins
```
...
```{block2, type='warning'}
**Très important\ :** pensez bien toujours à préparer et remainer vos données *avant* la séparation en set d'apprentissage et set de test. Ainsi, vous vous assurez d'avoir deux tableaux compatibles. Les points à prendre en condération ici sont\ : (1) je vérifie que ma variable à prédire est bien sous forme `factor` (utilisez `class(df$var)` pour le déterminer si le tableau s'appelle `df` et la variable s'appelle `var`, ensuite j'utilise `df$var <- as.factor(df$var)` si c'est nécessaire), (2) j'élimine les variables inutilisées avec `select()`, (3) je transforme les variables qui doivent l'être avec `mutate()` (transformation log, puissance, standardisation, etc.), (4) j'élimine éventuellement les données manquantes avec `drop-na()` tout à la fin et je vérifie ensuite qu'il me reste suffisamment de données pour construire et tester mon modèle. *Notez bien que nous avons fait tout cela plus haut avec notre jeu de données exemple `penguins`.*
...
Nous séparons nos observations en deux sets indépendants. Nous utilisons cependant la fonction `set.seed()` afin de fixer le début du générateur de nombres pseudo-aléatoires, ce qui donne une série de nombres qui ont les mêmes propriétés que des nombres tirés au sort, mais ce tirage au sort est *reproductible* d'une fois à l'autre.
...
```{r}
penguins_learn2 %<-%  select(penguins_learn, flipper_length, bill_length, species)
...
`r h5p(170, height = 270, toc = "Déploiement d'un algorithme.")`

##### À vous de jouer ! {.unnumbered}

```{r assign_C01Ia_21_ml1, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("C01Ia_21M_ml1", part = NULL,
    url = "https://github.com/BioDataScience-Course/C01Ia_21_ml1",
    course.ids = c(
      'S-BIOG-025'         = !"C01Ia_{YY}M_ml1"),
    course.urls = c(
      'S-BIOG-025'         = "https://classroom.github.com/a/1zEXuZQN"),
    course.starts = c(
      'S-BIOG-025'         = !"{W[4]+3} 13:00:00"),
    course.ends = c(
